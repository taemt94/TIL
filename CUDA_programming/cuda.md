...
# 2021/02/18
- CUDA 프로그래밍을 할 때 커널을 사용한 후에는 반드시 `cudaDeviceSynchronize()`를 사용하여 쓰레드 동기화를 해야 한다.
- 이를 잊고 그냥 넘어갈 경우, CPU에서 사용하는 함수이더라도 이전에는 실행이 되던 함수가 실행이 안되는 경우가 발생한다(대표적으로 `cudaMallocManaged`로 할당된 변수를 커널에서 값을 건드는 경우가 있다.).
- 코드가 복잡해지면 디버깅하여 이런 오류를 찾아내기가 매우 까다롭기 때문에 처음부터 쓰레드 동기화를 하는데 습관이 들 필요가 있다.